...
export class Mesdocuments extends Component {
    ...
    componentWillMount() {
        axios.get('/api/telechargement/user/'+this.state.user).then(response => {
            this.setState({
                data: response.data,
            });
        }).catch(error => {
        ...
        axios.get('/api/docs/user/'+this.state.user).then(response => {
            this.setState({
    ...
    actionTemplate(Column) {
        ...
        if (window.sessionStorage.getItem("role") == 2) {
            if (Column.user == window.sessionStorage.getItem("user")) {
                return <div>
                    <Button type="button" icon="pi pi-search" className="p-button-highlight" style={{ marginRight: '.5em' }} onClick={this.show.bind(Column)}></Button>
                    <Button type="button" icon="pi pi-download" className="p-button-success" style={{ marginRight: '.5em' }} onClick={this.download.bind(Column)}></Button>
                    <Button type="button" icon="pi pi-pencil" className="p-button-warning" style={{ marginRight: '.5em' }} onClick={(e) => this.edit(Column.id)}></Button>
    ...
    update() {
        ...
        params.append('user_id', this.state.item.user_id.id);
                    ...
                    <div className="card">
                        ...
                        <div className="content-section introduction">
                            <div className="feature-intro">
                                <h1>Mes documents</h1>
                                <p>Dans cette espace, vous pouvez voir les documents que vous avez partagé, ou téléchargé.</p>
                        ...
                        <div className="content-section implementation">
                            ...
                            paginator={true} rows={10} rowsPerPageOptions={[10,20,50]}>
                                ...
                                <Column field="categorie" header="Catégorie" filter={true} filterMatchMode="contains" style={{ width: '20em' }} />

                                <Column header="Actions" body={this.actionTemplate} style={{ textAlign: 'center', width: '20em' }} />

                            </DataTable>
                            <br/>
                            <h3>Documents partagés</h3>
                            <DataTable value={this.state.uploadData} globalFilter={this.state.globalFilter} header={header} responsive={true}
                            paginator={true} rows={10} rowsPerPageOptions={[10,20,50]}>
                                <Column field="nom" header="Nom" filter={true} filterMatchMode="contains" sortable={true} />
                                <Column field="categorie" header="Catégorie" filter={true} filterMatchMode="contains" style={{ width: '20em' }} sortable={true} />
